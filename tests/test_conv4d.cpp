...
NANO_CASE(3d_vs_4d_output)
{
        ...
        for (const auto dcols : {1, 2})
        {
                ...
                for (int i = 0; i < 3; ++ i)
                {
                        ...
                        op3d.output(idata, kdata, bdata, odata3);
                        op4d.output(idata, kdata, bdata, odata4);
...
NANO_CASE(3d_vs_4d_gparam)
{
        ...
        for (const auto dcols : {1, 2})
        {
                ...
                for (int i = 0; i < 3; ++ i)
                {
                        ...
                        op4d.output(idata, kdata4, bdata4, odata);// NB: needed to update the internal buffers!
                        op3d.gparam(idata, kdata3, bdata3, odata);
                        op4d.gparam(idata, kdata4, bdata4, odata);
...
NANO_CASE(3d_vs_4d_ginput)
{
        ...
        for (const auto dcols : {1, 2})
        {
                ...
                for (int i = 0; i < 3; ++ i)
                {
                        ...
                        op4d.output(idata4, kdata, bdata, odata);// NB: needed to update the internal buffers!
                        op3d.ginput(idata3, kdata, bdata, odata);
                        op4d.ginput(idata4, kdata, bdata, odata);

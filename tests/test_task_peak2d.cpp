...
NANO_CASE(regression)
{
        ...
        const auto config = json_writer_t().object(
                "type", "regression", "irows", irows, "icols", icols, "noise", 0, "count", count).str();
...
NANO_CASE(classification)
{
        ...
        const auto config = json_writer_t().object(
                "type", "classification", "irows", irows, "icols", icols, "noise", 0, "count", count).str();
        ...
        for (const auto p : {protocol::train, protocol::valid, protocol::test})
        {
                ...
                for (size_t i = 0; i < size; ++ i)
                {
                        ...
                        NANO_CHECK_CLOSE(min, 0, epsilon0<scalar_t>());
                        NANO_CHECK_CLOSE(target(0), (c < icols / 2 ? neg_target() : pos_target()), epsilon0<scalar_t>());
                        NANO_CHECK_CLOSE(target(1), (r < irows / 2 ? neg_target() : pos_target()), epsilon0<scalar_t>());
                }
        }

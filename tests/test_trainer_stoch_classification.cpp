...
NANO_BEGIN_MODULE(trainer_stoch_classification)
...
NANO_CASE(tune_and_train)
{
        ...
        for (const auto& solver : get_stoch_solvers().ids())
        {
                ...
                trainer->config(json_writer_t().object(
                        "epochs", 20, "solver", solver, "epsilon", epsilon1<scalar_t>()).str());
                ...
                NANO_CHECK_LESS(state.m_valid.m_error, epsilon1<scalar_t>());
        }
}

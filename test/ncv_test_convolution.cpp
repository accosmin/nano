...
void test(int isize, int ksize, int n_samples)
{
        ...
        if ((diff = std::fabs(sumf1cpu - sum1cpu)) > eps) { std::cout << "conv_test(1CPU) FAILED (diff = " << diff << ")!" << std::endl; }
        if ((diff = std::fabs(sumfxcpu - sum1cpu)) > eps) { std::cout << "conv_test(xCPU) FAILED (diff = " << diff << ")!" << std::endl; }
#ifdef NANOCV_HAVE_OPENCL

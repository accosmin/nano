...
NANO_BEGIN_MODULE(test_sampler)
...
NANO_CASE(evaluate)
{
        ...
        for (size_t f = 0; f < task.fsize(); ++ f)
        {
                ...
                const string_t header = "fold [" + nano::to_string(f + 1) + "/" + nano::to_string(task.fsize()) + "]";
                const string_t train_header = header + " protocol [" + nano::to_string(protocol::train) + "]";
                const string_t test_header = header + " protocol [" + nano::to_string(protocol::test) + "]";
                ...
                NANO_CHECK_EQUAL(train_batch_samples.size() + test_batch_samples.size(), n_samples);

                // check training samples
                NANO_CHECK(test::check_fold(train_batch_samples, train_fold));
                NANO_CHECK(test::check_fold(train_urand_samples, train_fold));

                nano::print(train_header + " batch", train_batch_samples);
                nano::print(train_header + " urand", train_urand_samples);

                // check test samples
                NANO_CHECK(test::check_fold(test_batch_samples, test_fold));
                NANO_CHECK(test::check_fold(test_urand_samples, test_fold));

                nano::print(test_header + " batch", test_batch_samples);
                nano::print(test_header + " urand", test_urand_samples);
        }
}

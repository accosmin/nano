...
namespace test
{
        ...
        void check_solution(const opt_state_t& state, const std::vector<std::pair<vector_t, scalar_t>>& solutions)
        {
...
//                        BOOST_CHECK_LE(dfx, epsilon);
//                        BOOST_CHECK_LE(dx, epsilon);
//                }
        }
                ...
                const std::vector<std::pair<vector_t, scalar_t>>& solutions)
        {
                ...
                for (batch_optimizer optimizer : optimizers)
                {
                        ...
                        check_solution(state, solutions);
                }
...
//        https://en.wikipedia.org/wiki/Test_functions_for_optimization
        ...
        // Beale function
        {
                ...
                const opt_opgrad_t fn_grad = [=] (const vector_t& x, vector_t& gx)
                {
                        ...
                        gx.resize(2);
                        gx(0) = 2.0 * (- z0 + z0 * b - z1 + z1 * b * b - z2 + z2 * b * b * b);
                        gx(1) = 2.0 * (z0 * a + z1 * a * 2.0 * b + z2 * a * 3.0 * b * b);

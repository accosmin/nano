...
do
        ...
        do
                fn_train ${dir_exp_svhn} ${model} ${params} ${!trainer} ${avg_crit} ${!model}${outlayer}
                fn_train ${dir_exp_svhn} ${model}_l2n ${params} ${!trainer} ${l2n_crit} ${!model}${outlayer}
                fn_train ${dir_exp_svhn} ${model}_var ${params} ${!trainer} ${var_crit} ${!model}${outlayer}
        done
done

# compare models
bash plot_models.sh ${dir_exp_svhn}/models.pdf ${dir_exp_svhn}/*.state

bash plot_models.sh ${dir_exp_svhn}/conv_max_models.pdf ${dir_exp_svhn}/conv_max_*.state
bash plot_models.sh ${dir_exp_svhn}/conv_min_models.pdf ${dir_exp_svhn}/conv_min_*.state
bash plot_models.sh ${dir_exp_svhn}/conv_avg_models.pdf ${dir_exp_svhn}/conv_avg_*.state

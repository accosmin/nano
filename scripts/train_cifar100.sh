...
outlayer="linear:dims=100;softmax:type=global;"
...
do
        ...
        do
                fn_train ${dir_exp_cifar100} ${model} ${params} ${!trainer} ${avg_crit} ${!model}${outlayer}
                fn_train ${dir_exp_cifar100} ${model}_l2n ${params} ${!trainer} ${l2n_crit} ${!model}${outlayer}
                fn_train ${dir_exp_cifar100} ${model}_var ${params} ${!trainer} ${var_crit} ${!model}${outlayer}
        done
done

# compare models
bash plot_models.sh ${dir_exp_cifar100}/models.pdf ${dir_exp_cifar100}/*.state

bash plot_models.sh ${dir_exp_cifar100}/conv_max_models.pdf ${dir_exp_cifar100}/conv_max_*.state
bash plot_models.sh ${dir_exp_cifar100}/conv_min_models.pdf ${dir_exp_cifar100}/conv_min_*.state
bash plot_models.sh ${dir_exp_cifar100}/conv_avg_models.pdf ${dir_exp_cifar100}/conv_avg_*.state

...
int main(int argc, char *argv[])
{
        ...
        zob::cmdline_t cmdline("describe a task");
        cmdline.add("", "task",                 ("tasks to choose from: " + zob::concatenate(task_ids, ", ")).c_str());
        ...
        if (!cmd_save_dir.empty())
        {
                for (size_t f = 0; f < task->fsize(); ++ f)
                {
                        ...
                        const string_t train_path = cmd_save_dir + "/" + cmd_task + "_train_fold" + zob::to_string(f + 1);
                        const string_t test_path = cmd_save_dir + "/" + cmd_task + "_test_fold" + zob::to_string(f + 1);

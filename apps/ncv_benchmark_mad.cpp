...
void test_mad(size_t size)
{
        ...
        const scalar_t mad    = test_mad(ncv::mad<scalar_t>, "mad", vec1, vec2, wei);
        const scalar_t madul2 = test_mad(ncv::mad_unroll<scalar_t, 2>, "madul2", vec1, vec2, wei);
        const scalar_t madul3 = test_mad(ncv::mad_unroll<scalar_t, 3>, "madul3", vec1, vec2, wei);
        const scalar_t madul4 = test_mad(ncv::mad_unroll<scalar_t, 4>, "madul4", vec1, vec2, wei);
        const scalar_t madul5 = test_mad(ncv::mad_unroll<scalar_t, 5>, "madul5", vec1, vec2, wei);
        const scalar_t madul6 = test_mad(ncv::mad_unroll<scalar_t, 6>, "madul6", vec1, vec2, wei);
        const scalar_t madul7 = test_mad(ncv::mad_unroll<scalar_t, 7>, "madul7", vec1, vec2, wei);
        const scalar_t madul8 = test_mad(ncv::mad_unroll<scalar_t, 8>, "madul8", vec1, vec2, wei);
        const scalar_t madeig = test_mad(ncv::tensor::mad_eig<scalar_t>, "madeig", vec1, vec2, wei);
        std::cout << std::endl;

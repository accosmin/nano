...
namespace nano
{
        ...
        {
                const auto v1 = std::isfinite(one.m_value) ? one.m_error_avg : std::numeric_limits<scalar_t>::max();
                const auto v2 = std::isfinite(two.m_value) ? two.m_error_avg : std::numeric_limits<scalar_t>::max();
                return v1 < v2;
        }

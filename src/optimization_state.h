#pragma once

#include "function.h"
#include "text/cast.h"

namespace nano
{
        class optimization_state_t;
        using ref_optimization_state_t = std::reference_wrapper<const optimization_state_t>;

        ///
        /// \brief compare two optimization states
        ///
        bool operator<(const optimization_state_t&, const optimization_state_t&);

        ///
        /// \brief optimization state described as:
        ///     current point (x),
        ///     function value (f),
        ///     gradient (g),
        ///     descent direction (d) &
        ///     line-search step (t)
        ///
        class optimization_state_t
        {
        public:

                enum class status
                {
                        converged,      ///< convergence criteria reached
                        max_iters,      ///< maximum number of iterations reached without convergence (default)
                        failed,         ///< optimization failed (e.g. line-search failed)
                        stopped         ///< user requested stop
                };

                ///
                /// \brief default constructor
                ///
                optimization_state_t() = default;

                ///
                /// \brief constructor
                ///
                explicit optimization_state_t(const tensor_size_t size) :
                        x(vector_t::Zero(size)),
                        g(vector_t::Zero(size)),
                        d(vector_t::Zero(size)),
                        f(std::numeric_limits<scalar_t>::max())
                {
                }

                ///
                /// \brief constructor
                ///
                optimization_state_t(const function_t& function, const vector_t& x0) :
                        optimization_state_t(x0.size())
                {
                        assert(function.size() == x.size());
                        update(function, x0);
                }

                ///
                /// \brief update current state (move to another position)
                ///
                void update(const function_t&, const vector_t& xx)
                {
                        x = xx;
                        f = function.eval(x, &g);
                }

                ///
                /// \brief update current state (move t along the chosen direction)
                ///
                void update(const function_t&, const scalar_t t)
                {
                        x.noalias() += t * d;
                        f = function.eval(x, &g);
                }

                ///
                /// \brief update current state (move t along the chosen direction,
                /// but the function value & gradient are already computed)
                ///
                void update(const function_t&, const scalar_t t, const scalar_t ft, const vector_t& gt)
                {
                        x.noalias() += t * d;
                        f = ft;
                        g = gt;
                }

                ///
                /// \brief check convergence: the gradient is relatively small
                ///
                bool converged(const scalar_t epsilon) const
                {
                        return convergence_criteria() < epsilon;
                }

                ///
                /// \brief convergence criteria: relative gradient
                ///
                scalar_t convergence_criteria() const
                {
                        return g.lpNorm<Eigen::Infinity>() / (1 + std::fabs(f));
                }

                ///
                /// \brief check divergence
                ///
                operator bool() const
                {
                        return std::isfinite(f) && std::isfinite(convergence_criteria());
                }

                // attributes
                vector_t        x, g, d;                        ///< parameter, gradient, descent direction
                scalar_t        f{0};                           ///< function value, step size
                status          m_status{status::max_iters};    ///< optimization status (todo: does it make sense to have it here?!)
        };

        inline bool operator<(const optimization_state_t& one, const optimization_state_t& two)
        {
                const auto f1 = std::isfinite(one.f) ? one.f : std::numeric_limits<scalar_t>::max();
                const auto f2 = std::isfinite(two.f) ? two.f : std::numeric_limits<scalar_t>::max();

                return f1 < f2;
        }

        template <>
        inline enum_map_t<optimization_state_t::status> enum_string<optimization_state_t::status>()
        {
                return
                {
                        { optimization_state_t::status::converged,   "converged" },
                        { optimization_state_t::status::max_iters,   "max_iters" },
                        { optimization_state_t::status::failed,      "failed" },
                        { optimization_state_t::status::stopped,     "stopped" }
                };
        }

        inline std::ostream& operator<<(std::ostream& os, const optimization_state_t::status status)
        {
                return os << to_string(status);
        }
}

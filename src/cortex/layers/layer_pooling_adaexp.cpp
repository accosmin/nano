...
#include "layer_pooling_adaexp.h"

namespace nano
{
        pooling_adaexp_layer_t::pooling_adaexp_layer_t(const string_t& parameters) :
                layer_t(parameters)
        {
        }

        tensor_size_t pooling_adaexp_layer_t::resize(const tensor3d_t& tensor)
        {
        ...
        void pooling_adaexp_layer_t::zero_params()
        {
                tensor::set_zero(m_wdata);
        }

        void pooling_adaexp_layer_t::random_params(scalar_t min, scalar_t max)
        {
                tensor::set_random(nano::random_t<scalar_t>(min, max), m_wdata);
        }

        scalar_t* pooling_adaexp_layer_t::save_params(scalar_t* params) const
        {
                return tensor::to_array(params, m_wdata);
        }

        const scalar_t* pooling_adaexp_layer_t::load_params(const scalar_t* params)
        {
                return tensor::from_array(params, m_wdata);
        }

        const tensor3d_t& pooling_adaexp_layer_t::output(const tensor3d_t& input)
        {
        ...
        const tensor3d_t& pooling_adaexp_layer_t::ginput(const tensor3d_t& output)
        {
        ...
        void pooling_adaexp_layer_t::gparam(const tensor3d_t& output, scalar_t* gradient)
        {
                assert(odims() == output.size<0>());
                assert(orows() == output.size<1>());
                assert(ocols() == output.size<2>());

...
namespace ncv
{
        ...
        void stochastic_trainer_t::sgd(const task_t& task, const samples_t& samples, const loss_t& loss,
                const model_t& model, vector_t& x, scalar_t gamma, scalar_t lambda, size_t iterations) const
        {
                const rmodel_t rmodel = model.clone();
                ...
                for (size_t iteration = 0; iteration < iterations; iteration ++)
                {
                        ...
                        rmodel->zero_grad();
                        ncv::lvgrad(task, sample, loss, *rmodel);

                        x -= learning_rate(gamma, lambda, iteration) * rmodel->grad();
                }
        }
        ...
        bool stochastic_trainer_t::train(const task_t& task, const fold_t& fold, const loss_t& loss, model_t& model) const
        {
                ...
                {
                        log_info() << "stochastic trainer: state [epoch = " << epoch << "/" << epochs
...
//                        log_info() << "stochastic trainer: epoch finished, average loss = "
